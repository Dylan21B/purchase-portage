angular
    ...
    .controller("reservationsItineraryCtrl", function ($scope, $routeParams, timeSlotFactory, reservationsFactory, invoiceFactory) {
        ...
        function brightSize(num1, num2) {
            let clearLi = document.querySelectorAll("li.slot")
            clearLi.forEach(element => {
                element.classList.remove("bright")
            });
            for (let i = 0 + num1; i < num2 + num1; i++) {
                switch (i) {
                    case 1:
        ...
        $scope.brightLabor = function (divNum) {
            ...
            } else if (divNum > 2 && divNum < 35) {
                startDelivery = divNum - 2
                endDelivery = divNum + divSize + 2
            } else if (divNum = 35) {
                startDelivery = divNum - 3
                endDelivery = 36
            } else {
                startDelivery = divNum - 4
                endDelivery = 36
            }
